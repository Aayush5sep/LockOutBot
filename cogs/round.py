...
class Round(commands.Cog):
  ...
  def make_round_embed(self, ctx):
    ...
    embed.set_footer(
      text=
      "Use the prefix . before each command. For detailed usage about a particular command, type .help match <command>"
    )
    embed.add_field(
    ...
    embed.add_field(
      ...
      value=
      f"[Invite](https://discord.com/oauth2/authorize?client_id=669978762120790045&permissions=0&scope=bot)",
      inline=True)
  ...
  async def challenge(self, ctx, *users: discord.Member):
    ...
    for i in users:
      ...
      if self.db.in_a_round(ctx.guild.id, i.id):
        await discord_.send_message(
          ctx, f"{i.mention} is already in a round or registered in some!")
        return
    ...
    problems = await codeforces.find_problems(
      [self.db.get_handle(ctx.guild.id, x.id) for x in users], rating)
    if not problems[0]:
  ...
  async def setup(self, ctx, division_name: str):
    ...
    if len(division_name) not in range(1, 51):
      await discord_.send_message(
        ctx, "The division name should be 50 character max")
      return
    if any([not ch.isalnum() and ch != ' ' for ch in division_name]):
      await discord_.send_message(
        ctx, "The division name should contain only alpha-numeric characters")
      return
    ...
    self.db.add_to_ongoing_round(ctx, [], rating, points, problems, duration,
                                 repeat, 0, tournament)
  ...
  async def register(self, ctx):
    ...
    try:
      ...
      await discord_.send_message(
        ctx, "There is no ongoing round in the server currently")
      return
    except:
    ...
    if self.db.in_registrants(ctx.guild.id, ctx.author.id):
      await discord_.send_message(ctx,
                                  "You have already registered for the round")
      return
    if self.db.in_a_round(ctx.guild.id, ctx.author.id):
    ...
    if len(registrants) == MAX_REGISTRANTS:
      await discord_.send_message(
        ctx, "The round has already reached its max registrants limit!")
      return
    ...
    await ctx.send(embed=discord.Embed(
      description=
      f"Successfully registered for the round. `{MAX_REGISTRANTS-len(registrants)-1}` slots left.",
      color=discord.Color.green()))
  ...
  async def unregister(self, ctx):
    ...
    await ctx.send(embed=discord.Embed(
      description=
      f"Successfully unregistered from the round. `{MAX_REGISTRANTS - len(registrants) + 1}` slots left.",
      color=discord.Color.green()))

  @round.command(name="_unregister",
                 brief="Forcefully unregister someone from the round")
  async def _unregister(self, ctx, *, handle: str):
    ...
    res = self.db.remove_reg_by_handle(ctx.guild.id,
                                       ctx.message.mentions[0].id)
    if not res:
      await discord_.send_message(
        ctx,
        f"The user with handle `{handle}` has not registered for the round")
      return
    await ctx.send(embed=discord.Embed(
      description=
      f"Successfully unregistered from the round. `{MAX_REGISTRANTS - len(registrants) + 1}` slots left.",
  ...
  @round.command(name="registrants",
                 brief="View the list of users who have registered the round")
  async def registrants(self, ctx):
    ...
    await paginator.Paginator([[
      str(i + 1),
      self.db.get_handle(ctx.guild.id, registrants[i].discord_id),
      str(self.db.get_handle_info(ctx.guild.id, registrants[i].discord_id)[3])
    ] for i in range(len(registrants))], ["S No.", "Handle", "Rating"],
                              "Registrants for the Lockout round",
                              15).paginate(ctx, self.client)
  ...
  async def ongoing(self, ctx):
    ...
    text = '\n'.join(content[currPage *
                             ROUNDS_PER_PAGE:min(len(content), (currPage + 1) *
                                                 ROUNDS_PER_PAGE)])
    ...
    while True:
      try:
        ...
        if reaction.emoji == "⏮":
          ...
          currPage = totPage - 1
        text = '\n'.join(
          content[currPage * ROUNDS_PER_PAGE:min(len(content), (currPage + 1) *
                                                 ROUNDS_PER_PAGE)])
  ...
  async def delete_(self, ctx):
    ...
    resp = await discord_.get_time_response(
      self.client, ctx,
      "Are you sure you want to delete the round? This action is irreversable. Type `1` for yes and `0` for no",
      30, ctx.author, [0, 1])
  ...
  async def recent(self, ctx, user: discord.Member = None):
    data = self.db.get_recent_rounds(ctx.guild.id,
  ...
  async def begin(self, ctx):
    ...
    if round_info.active != 0:
      await discord_.send_message(
        ctx,
        f"The round has already begun! Type `.round matches` or `.round info` to view details about the round"
      )
      return
    ...
    if not registrants or len(registrants) < 2:
      await discord_.send_message(ctx,
                                  "Not enough registrants for the round yet")
      return
    ...
    problems = await codeforces.find_problems(
      [self.db.get_handle(ctx.guild.id, x) for x in users],
      [int(rt) for rt in round_info.rating.split()])
    if not problems[0]:
      await discord_.send_message(ctx, problems[1])
      return
  ...
  async def update(self, ctx):
    await ctx.send(
      embed=discord.Embed(description="Updating rounds for this server",
                          color=discord.Color.green()))
        ...
        for i in range(len(resp[0])):
          if len(resp[0][i]):
            await channel.send(embed=discord.Embed(
              description=
              f"{' '.join([(await discord_.fetch_member(ctx.guild, m)).mention for m in resp[0][i]])} has solved problem worth **{round.points.split()[i]}** points",
              color=discord.Color.blue()))

        if not resp[1] and resp[2]:
          new_info = self.db.get_round_info(round.guild, round.users)
          await channel.send(
            embed=await discord_.round_problems_embed(new_info))

        if resp[1]:
          round_info = self.db.get_round_info(round.guild, round.users)
          ranklist = await updation.round_score(
            list(map(int, round_info.users.split())),
            list(map(int, round_info.status.split())),
            list(map(int, round_info.times.split())))
          eloChanges = await elo.calculateChanges([[
            (await discord_.fetch_member(ctx.guild, user.id)), user.rank,
          ...
          if round_info.tournament == 1:
            ...
            if ranklist[1].rank == 1 and tournament_info.type != 2:
              ...
              res = await tournament_helper.validate_match(
                round_info.guild, ranklist[0].id, ranklist[1].id, self.api,
                self.db)
              if not res[0]:
                await discord_.send_message(
                  channel, res[1] +
                ...
                match_resp = await self.api.post_match_results(
                  res[1]['tournament_id'], res[1]['match_id'], scores,
                ...
                if not match_resp or 'errors' in match_resp:
                  ...
                  continue
                winner_handle = self.db.get_handle(round_info.guild,
                                                   ranklist[0].id)
                await discord_.send_message(
                  channel,
                ...
                if await tournament_helper.validate_tournament_completion(
                  ...
                  await channel.send(
                    embed=await tournament_helper.tournament_over_embed(
  ...
  @round.command(name="custom",
                 brief="Challenge to a round with custom problemset")
